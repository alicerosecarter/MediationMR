...
foreach out in sbp hyperten CVD_inc {
	...
	putexcel set 202002_results_bmi, sheet(`out') modify
...
foreach out in sbp hyperten CVD_inc {
	...
	ivreg2 `out' (eduyears_scaled zbmi = ea_weighted bmi_weighted) $PCs $max_covar
...
regress eduyears_scaled ea_weighted bmi_weighted $max_covar $PCs
predict dir_eabmi, xb
regress zbmi bmi_weighted ea_weighted $max_covar $PCs
...
foreach out in sbp hyperten CVD_inc {
	...
	scalar total_sbp = _b[eduyears_scaled]
	ivreg2 sbp (eduyears_scaled zbmi = ea_weighted bmi_weighted) $PCs $max_covar
	...
	scalar diff_prop_bmi_sbp = (indirect_diff_sbp/total_sbp)*100 
	}
	if (`out' == hyperten) {
	ivreg2 hyperten (eduyears_scaled  = ea_weighted ) $PCs $max_covar
	scalar total_hyperten = _b[eduyears_scaled]
	ivreg2 hyperten (eduyears_scaled zbmi = ea_weighted bmi_weighted) $PCs $max_covar
	...
	scalar diff_prop_bmi_CVD_inc = (indirect_diff_hyperten/total_hyperten)*100 
	}
	if (`out' == CVD_inc) {
	ivreg2 CVD_inc (eduyears_scaled  = ea_weighted ) $PCs $max_covar
	scalar total_CVD_inc = _b[eduyears_scaled]
	ivreg2 CVD_inc (eduyears_scaled zbmi = ea_weighted bmi_weighted) $PCs $max_covar
	...
	scalar diff_prop_bmi_hyperten = (indirect_diff_CVD_inc/total_CVD_inc)*100 
	}
}
end	
foreach out in sbp hyperten CVD_inc {
...
bootstrap prop_med = prop_bmi_`out', reps(100) nodrop : boot  // indirect effect
         matrix results = r(table)
         local proportion = results[1,1]
         local proportion_LCI = results[5,1]
         local proportion_UCI = results[6,1]
